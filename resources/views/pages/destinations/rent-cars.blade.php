...
@section('content')
    ...
    <section id="explore_area" class="section_padding">
        <div class="container">
            <div class="row">
                <div class="col-lg-3">
                    <form action="{{route('rent-cars', [$current_language->code, $regionGroup->slug])}}" method="GET">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                                        ...
                                        <div class="row">
                                            <div class="col-lg-4">
                                                <div class="cruise_item_img">
                                                    <img src="/storage/public/media/{{$rent->base_image->path}}" alt="{{$rent->title}}">
                                                </div>
                                            </div>
                                            <div class="col-lg-8">
                                                <div class="cruise_item_inner_content">
                                                    <div class="cruise_content_top_wrapper">
                                                        <div class="cruise_content_top_left">
                                                            <ul>
                                                                <li>{{$rent->destination?->name}}</li>
                                                            </ul>
                                                            <h4>{{$rent->brand?->title}} / {{$rent->title}}</h4>
                                                            <p><i class="fas fa-map-marker-alt"></i> {{$rent->address?->name}}, {{$rent->destination?->name}}</p>
                                                        </div>
                                                    </div>
                                                    <div class="cruise_content_middel_wrapper">
                                                        <div class="cruise_content_middel_left">
                                                            <p>{{shortenText(cleanBody($rent->description), 70)}}</p>
                                                        </div>
                                                        <div class="cruise_content_middel_right">
                                                            <h3>{{$rent->daily_price}}{{__('currency')}}</h3>
                                                        </div>
                                                    </div>
                                                    <div class="cruise_content_bottom_wrapper">
                                                        <div class="cruise_content_bottom_left">
                                                            <ul>
                                                                <li class="mb-2">{{$rent->daily_price}} {{__('currency')}} / {{__('per_day')}}</li>
                                                                <li class="mb-2">{{$rent->weekly_price}} {{__('currency')}} / {{__('per_week')}}</li>
                                                                <li class="mb-2">{{$rent->monthly_price}} {{__('currency')}} / {{__('per_month')}}</li>
                                                                <li class="mb-2">{{$rent->color}}</li>
                                                        ...
                                                        <div class="cruise_content_bottom_right">
                                                            <a href="{{route('rent-cars-show', [$current_language->code, $rent->slug])}}" class="btn btn_theme btn_md">{{__('view_details')}}</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div class="load_more_flight">
                                {{$rents->links('vendor.pagination.bootstrap-4', ['title' => $title, 'min_price' => $min_price, 'max_price' => $max_price, 'facilities' => $facilitiesArray, 'page' => $page])}}
                            </div>
                        </div>
                    </div>
                </div>
