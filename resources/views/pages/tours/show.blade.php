...
@section('content')
    ...
    @push('common-banner')
        @include('components.common-banner', [
            ...
            'image' => '/storage/public/media/'.$tour->cover_image->path
        ])
    @endpush
                    ...
                    <div class="tour_details_leftside_wrapper">
                        <div class="tour_details_heading_wrapper">
                            <div class="tour_details_top_heading">
                                <h2>{{$tour->title}}</h2>
                                @if($tour->address)
                                    <h5><i class="fas fa-map-marker-alt"></i> {{$tour->address?->name}}</h5>
                                @endif
                            </div>
                            ...
                            <div class="tour_details_top_heading_right">
                                <h4>{{getRatingStatus($tour->rating)}}</h4>
                                <h6>{{$tour->rating}}/5</h6>
                                <p>({{$tour->reviews->count()}} {{__('reviews')}})</p>
                            </div>
                        </div>
                        <div class="tour_details_img_wrapper">
                            <div class="slider-for">
                                @foreach($tour->additional_images as $key => $image)
                                    <div>
                                        <img src="/storage/public/media/{{$image->path}}" style="width: 930px; height: 450px; object-fit: contain" alt="{{$tour->title}} {{$key + 1}}">
                                    </div>
                                @endforeach
                            </div>
                            <div class="slider-nav">
                                @foreach($tour->additional_images as $key => $image)
                                    <div>
                                        <img src="/storage/public/media/{{$image->path}}" style="width: 145px; height: 130px; object-fit: contain" alt="{{$tour->title}} {{$key + 1}}">
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="tour_details_boxed">
                            ...
                            <div class="map_area">
                                <iframe
                                    ...
                                    marginheight="0"
                                    marginwidth="0"
                                    src="https://maps.google.com/maps?q={{$tour->address->latitude}},{{$tour->address->longitude}}&hl={{$current_language->code}}&z={{$tour->address->zoom}}&amp;output=embed">
                                </iframe>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="tour_detail_right_sidebar">
                            <div class="tour_details_right_boxed" style="border-bottom-left-radius: 0; border-bottom-right-radius: 0">
                                ...
                                <div class="tour_package_bar_price">
                                    <h6>
                                        <del>{{$tour->price}} {{__('currency')}}</del>
                                    </h6>
                                    <h3>{{$tour->price}} {{__('currency')}}<sub> / {{__('per_person')}}</sub></h3>
                                </div>
                            </div>
                        ...
                        <div class="write_review_inner_boxed">
                            <form action="{{route('storeReview', [$current_language->code])}}" method="post" id="news_comment_form">
                                ...
                                <div class="row">
                                    ...
                                    <input type="hidden" value="{{$tour->id}}" name="model_id">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="promotional_tour_slider owl-theme owl-carousel dot_style">
                            @foreach($relatedTours as $key => $relatedTour)
                                <div class="theme_common_box_two img_hover">
                                    <div class="theme_two_box_img">
                                        <img src="/storage/public/media/{{$relatedTour->base_image?->path}}" alt="{{$relatedTour->title}}">
                                        <p>
                                            <i class="fas fa-map-marker-alt"></i>{{$relatedTour->destination?->name}}
                                        </p>
                                    </div>
                                    <div class="theme_two_box_content">
                                        <h4>
                                            <a href="{{route('hotels-show', [$current_language->code, $relatedTour->slug])}}">{{$relatedTour->address?->name}}</a>
                                        </h4>
                                        <p>
                                            <span class="review_rating">{{$relatedTour->rating}} / 5 {{getRatingStatus($relatedTour->rating)}}</span>
                                            <span class="review_count">({{$relatedTour->reviews->count()}} {{__('reviews')}})</span>
                                        </p>
                                        @if($relatedTour->price > 0)
                                            <h3>{{$relatedTour->price}} {{__('currency')}}</h3>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
    ...
    @include('components.book', ['model_type' => \App\Models\Tour::class, 'model_id' => $tour->id])
@endsection
