...
@section('content')
    ...
    @push('common-banner')
        @include('components.common-banner', [
            ...
            'image' => '/storage/public/media/'.$hotel->cover_image->path
        ])
    @endpush
                    ...
                    <div class="tour_details_leftside_wrapper">
                        <div class="tour_details_heading_wrapper">
                            <div class="tour_details_top_heading">
                                <h2>{{$hotel->title}}</h2>
                                @if($hotel->address)
                                    <h5><i class="fas fa-map-marker-alt"></i> {{$hotel->address?->name}}</h5>
                                @endif
                            </div>
                            ...
                            <div class="tour_details_top_heading_right">
                                <h4>{{getRatingStatus($hotel->rating)}}</h4>
                                <h6>{{$hotel->rating}}/5</h6>
                                <p>({{$hotel->reviews->count()}} {{__('reviews')}})</p>
                            </div>
                        </div>
                        <div class="tour_details_img_wrapper">
                            <div class="slider-for">
                                @foreach($hotel->additional_images as $key => $image)
                                    <div>
                                        <img src="/storage/public/media/{{$image->path}}"
                                             style="width: 930px; height: 450px; object-fit: contain"
                                             alt="{{$hotel->title}} {{$key + 1}}">
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="tour_details_boxed">
                            ...
                            <div class="map_area">
                                <iframe
                                    ...
                                    marginheight="0"
                                    marginwidth="0"
                                    src="https://maps.google.com/maps?q={{$hotel->address->latitude}},{{$hotel->address->longitude}}&hl={{$current_language->code}}&z={{$hotel->address->zoom}}&amp;output=embed">
                                </iframe>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="tour_detail_right_sidebar">
                            <div class="tour_details_right_boxed" style="border-bottom-left-radius: 0; border-bottom-right-radius: 0">
                                ...
                                <div class="tour_package_bar_price">
                                    <h6>
                                        <del>{{$hotel->price}} {{__('currency')}}</del>
                                    </h6>
                                    <h3>{{$hotel->price}} {{__('currency')}}<sub> / {{__('per_person')}}</sub></h3>
                                </div>
                            ...
                            <div class="tour_select_offer_bar_bottom mt-3">
                                <a href="{{$hotel->booking_url}}"
                                   target="_blank"
                                   class="btn btn_theme btn_md w-100"> {{__('see_booking_com')}}
                                </a>
                            </div>
                        </div>
                        <div class="tour_detail_right_sidebar">
                        ...
                        <div class="write_review_inner_boxed">
                            <form action="{{route('storeReview', [$current_language->code])}}" method="post"
                                ...
                                <div class="row">
                                    ...
                                    <input type="hidden" value="{{$hotel->id}}" name="model_id">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="promotional_tour_slider owl-theme owl-carousel dot_style">
                        @foreach($relatedHotels as $key => $relatedHotel)
                            <div class="theme_common_box_two img_hover">
                                <div class="theme_two_box_img">
                                    <img src="/storage/public/media/{{$relatedHotel->base_image?->path}}" alt="{{$relatedHotel->title}}">
                                    <p>
                                        <i class="fas fa-map-marker-alt"></i>{{$relatedHotel->destination?->name}}
                                    </p>
                                </div>
                                <div class="theme_two_box_content">
                                    <h4>
                                        <a href="{{route('hotels-show', [$current_language->code, $relatedHotel->slug])}}">{{$relatedHotel->address?->name}}</a>
                                    </h4>
                                    <p>
                                        <span class="review_rating">{{$relatedHotel->rating}} / 5 {{getRatingStatus($relatedHotel->rating)}}</span>
                                        <span class="review_count">({{$relatedHotel->reviews->count()}} {{__('reviews')}})</span>
                                    </p>
                                    <h3>{{$relatedHotel->price}} {{__('currency')}}</h3>
                                </div>
                            </div>
                        @endforeach
                    </div>
    ...
    @include('components.book', ['model_type' => \App\Models\Place::class, 'model_id' => $hotel->id])
@endsection
