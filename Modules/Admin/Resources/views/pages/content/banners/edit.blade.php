...
@section('content')
    @push('breadcrumb-buttons')
        <div class="page-title d-flex">
            <h4>
                <a class="text-black-50" href="{{route('content.banner.index')}}"> Bannerə düzəliş et</a>
            </h4>
    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{{route('content.banner.update', [$banner->id])}}" method="post" enctype="multipart/form-data" id="submit-form">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade show active" id="general">
                                <fieldset class="mb-3">
                                        ...
                                        <div class="col-lg-10">
                                            <div class="form-group-feedback form-group-feedback-right">
                                                <select id="banner_type_id" class="select-search @if($errors->has('banner_type_id')) border-danger @endif" name="type_id">
                                                    <option value="0" @if($banner->type_id === 0) selected @endif>Seçilməyib</option>
                                                    @foreach($bannerTypes as $bannerType)
                                                        <option value="{{$bannerType->id}}" @if($bannerType->id === $banner->type_id) selected @endif>{{$bannerType->name}}</option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-lg-10">
                                            <div class="form-group-feedback form-group-feedback-right">
                                                <select id="position" class="select-search @if($errors->has('position')) border-danger @endif" name="position">
                                                    <option value="0" @if($banner->position === 0) selected @endif>Seçilməyib</option>
                                                    @foreach($positions as $position)
                                                        <option value="{{$position}}" @if($position === $banner->position) selected @endif>{{$position}}</option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-lg-10">
                                            <div class="form-group-feedback form-group-feedback-right">
                                                <select id="effect" class="select-search @if($errors->has('effect')) border-danger @endif" name="effect">
                                                    <option value="0" @if($banner->effect === 0) selected @endif>Seçilməyib</option>
                                                    @foreach(animateList() as $effect)
                                                        <option value="{{$effect}}" @if($effect === $banner->effect) selected @endif>{{$effect}}</option>
                                                    @endforeach
                                                </select>
                                    ...
                                    @if(isset($banner->base_image))
                                        <div class="form-group row">
                                            <div class="col-form-label col-lg-2 font-weight-semibold"></div>
                                            <div class="col-lg-10">
                                                <div class="form-group-feedback form-group-feedback-right">
                                                    <img style="width: 200px; height: 100px; object-fit: contain;" src="/storage/{{$banner->base_image->path}}" alt="Image Preview">
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                                    ...
                                    @if(isset($banner->additional_images))
                                        <div class="form-group row">
                                            <div class="col-form-label col-lg-2 font-weight-semibold"></div>
                                            <div class="col-lg-10">
                                                <div class="form-group-feedback form-group-feedback-right">
                                                    <img style="width: 100px; height: 100px; object-fit: contain;" src="/storage/{{$banner->additional_images->first()->path ?? ''}}" alt="Image Preview">
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                                            ...
                                            <div class="form-group-feedback form-group-feedback-right">
                                                <div class="form-check form-check-switchery">
                                                    <label class="form-check-label">
                                                        <input type="checkbox" id="status" class="form-check-input-switchery" @if($banner->status === 1) checked="checked" @endif name="status" data-fouc>
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
                                                    ...
                                                    <div class="col-lg-10">
                                                        <div class="form-group-feedback form-group-feedback-right">
                                                            <input name="button_title[{{$language->code}}]" id="button_title_{{$language->code}}"  type="text"
                                                                   value="{{(old('button_title')[$language->code] ?? null) ?? ($translations[$language->code]['button_title'] ?? '')}}"
                                                    ...
                                                    <div class="col-lg-10">
                                                        <div class="form-group-feedback form-group-feedback-right">
                                                            <input name="button_url[{{$language->code}}]" id="button_url_{{$language->code}}"  type="text"
                                                                   value="{{(old('button_url')[$language->code] ?? null) ?? ($translations[$language->code]['button_url'] ?? '')}}"
                                                    ...
                                                    <div class="col-lg-10">
                                                        <div class="form-group-feedback form-group-feedback-right">
                                                            <input name="button_icon[{{$language->code}}]" id="button_icon_{{$language->code}}"  type="text"
                                                                   value="{{(old('button_icon')[$language->code] ?? null) ?? ($translations[$language->code]['button_icon'] ?? '')}}"
