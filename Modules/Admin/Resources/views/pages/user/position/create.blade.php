...
@section('content')
    @push('breadcrumb-buttons')
        <div class="page-title d-flex">
            <h4>
                <a class="text-black-50" href="{{route('user.position.index')}}"> Pozisiya əlavə et</a>
            </h4>
    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{{route('user.position.store')}}" method="post" enctype="multipart/form-data" id="submit-form">
                                    ...
                                    <div class="form-group-feedback form-group-feedback-right">
                                        <select id="parent_id" class="select-search @if($errors->has('parent_id')) border-danger @endif" name="parent_id">
                                            ...
                                            @foreach($positions as $value)
                                                <option value="{{$value->id}}" @if($value->id === old('parent_id')) selected @endif>{{$value->position_name}}</option>
                                            @endforeach
                                        </select>
