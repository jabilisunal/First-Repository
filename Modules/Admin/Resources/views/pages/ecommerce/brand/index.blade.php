...
@section('content')
    @push('breadcrumb-buttons')
        <div class="page-title d-flex">
            <h4>
                <a class="text-black-50" href="{{route('ecommerce.brand.index')}}"> Brendlər</a>
            </h4>
            <a href="#" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
        </div>
        @can('content:post-create')
        <div class="header-elements d-none">
            <a href="{{route('ecommerce.brand.create')}}" type="button" class="btn bg-teal-400 btn-labeled btn-labeled-left">
                <b><i class="icon-plus3"></i></b> Brend əlavə et
            </a>
        </div>
        @endcan
    @endpush
                                ...
                                <td>
                                    <div class="list-icons">
                                        @can('content:post-show')
                                            <a target="_blank" href="{{route('ecommerce.brand.show', [$value->id])}}" class="list-icons-item"><i class="icon-eye"></i></a>
                                        @endcan
                                        @can('content:post-edit')
                                            <a href="{{route('ecommerce.brand.edit', [$value->id])}}" class="list-icons-item"><i class="icon-pencil7"></i></a>
                                        @endcan
                ...
                $(document).on('click', '.btn-remove', function () {
                    ...
                    swalInit({
                        ...
                        if(result.value) {
                            ...
                            $.ajax({
                                type: 'DELETE',
                                url: '/admin/ecommerce/brand/'+id,
                                data: {
                                    _token: $('meta[name="csrf_token"]').attr('content')
                                },
                                success: function (data) {
                                    if (data.status) {
                                        window.location='{{route('ecommerce.brand.index')}}'
                                    } else {
