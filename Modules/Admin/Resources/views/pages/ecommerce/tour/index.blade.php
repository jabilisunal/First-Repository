...
@section('content')
    @push('breadcrumb-buttons')
        <div class="page-title d-flex">
            <h4>
                <a class="text-black-50" href="{{route('ecommerce.tours.index')}}"> Tours</a>
            </h4>
            <a href="#" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
        </div>
        @can('ecommerce:tours-create')
        <div class="header-elements d-none">
            <a href="{{route('ecommerce.tours.create')}}" type="button" class="btn bg-teal-400 btn-labeled btn-labeled-left">
                <b><i class="icon-plus3"></i></b> Tour əlavə et
            </a>
        </div>
        @endcan
    @endpush
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="list-icons">
                                        @can('ecommerce:tours-show')
                                            <a target="_blank" href="{{route('ecommerce.tours.show', [$value->id])}}" class="list-icons-item"><i class="icon-eye"></i></a>
                                        @endcan
                                        @can('ecommerce:tours-edit')
                                            <a href="{{route('ecommerce.tours.edit', [$value->id])}}" class="list-icons-item"><i class="icon-pencil7"></i></a>
                                        @endcan
                                        @can('ecommerce:tours-destroy')
                ...
                $(document).on('click', '.btn-remove', function () {
                    ...
                    swalInit({
                        ...
                        if(result.value) {
                            ...
                            $.ajax({
                                type: 'DELETE',
                                url: '/admin/ecommerce/tours/'+id,
                                data: {
                                    _token: $('meta[name="csrf_token"]').attr('content')
                                },
                                success: function (data) {
                                    if (data.status) {
                                        window.location='{{route('ecommerce.tours.index')}}'
                                    } else {
