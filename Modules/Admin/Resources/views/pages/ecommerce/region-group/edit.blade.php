...
@section('content')
    @push('breadcrumb-buttons')
        <div class="page-title d-flex">
            <h4>
                <a class="text-black-50" href="{{route('ecommerce.region-groups.index')}}"> Brendə düzəliş et</a>
            </h4>
    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{{route('ecommerce.region-groups.update', [$regionGroup->id])}}" method="post" enctype="multipart/form-data" id="submit-form">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade show active" id="general">
                                <fieldset class="mb-3">
                                            ...
                                            <div class="form-group-feedback form-group-feedback-right">
                                                <select class="form-control multiselect-full-featured " id="destinations" name="destinations[]" multiple="multiple" data-fouc>
                                                    @foreach($destinations as $key => $value)
                                                        <option value="{{$value->id}}" @if(in_array($value->id, $regionGroup->destinations->pluck('id')->toArray())) selected @endif>{{$value->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                    ...
                                    @if($regionGroup->base_image->path)
                                        <div class="form-group row">
                                            <div class="col-form-label col-lg-2 font-weight-semibold"></div>
                                            <div class="col-lg-10">
                                                <div class="form-group-feedback form-group-feedback-right">
                                                    <img style="width: 200px; height: 100px; object-fit: contain;" src="/storage/{{$regionGroup->base_image->path}}" alt="Image Preview">
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                                            ...
                                            <div class="form-group-feedback form-group-feedback-right">
                                                <div class="form-check form-check-switchery">
                                                    <label class="form-check-label">
                                                        <input type="checkbox" id="status" class="form-check-input-switchery" @if($regionGroup->status === 1) checked="checked" @endif name="status" data-fouc>
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
