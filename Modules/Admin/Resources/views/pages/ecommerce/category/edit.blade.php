...
@section('content')
    @push('breadcrumb-buttons')
        <div class="page-title d-flex">
            <h4>
                <a class="text-black-50" href="{{route('ecommerce.category.index')}}"> Kateqoriyaya düzəliş et</a>
            </h4>
    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{{route('ecommerce.category.update', [$category->id])}}" method="post" enctype="multipart/form-data" id="submit-form">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade show active" id="general">
                                <fieldset class="mb-3">
                                        ...
                                        <div class="col-lg-10">
                                            <div class="form-group-feedback form-group-feedback-right">
                                                <select id="parent_id" class="select-search @if($errors->has('parent_id')) border-danger @endif" name="parent_id">
                                                    <option value="" @if($category->parent_id === null) selected @endif>Seçilməyib</option>
                                                    @foreach($categories as $mene)
                                                        <option value="{{$mene->id}}" @if($mene->id === $category->parent_id) selected @endif>{{$mene->title}}</option>
                                                    @endforeach
                                                </select>
                                    ...
                                    @if($category->base_image->path)
                                        <div class="form-group row">
                                            <div class="col-form-label col-lg-2 font-weight-semibold"></div>
                                            <div class="col-lg-10">
                                                <div class="form-group-feedback form-group-feedback-right">
                                                    <img style="width: 100px; height: 100px; object-fit: contain;" src="/storage/{{$category->base_image->path}}" alt="Image Preview">
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                                    ...
                                    @if(count($category->additional_images) > 0)
                                        <div class="form-group row">
                                            <div class="col-form-label col-lg-2 font-weight-semibold"></div>
                                            <div class="col-lg-10">
                                                <div class="form-group-feedback form-group-feedback-right">
                                                    <img style="width: 100px; height: 100px; object-fit: contain;" src="/storage/{{$category->additional_images->first()->path ?? ''}}" alt="Image Preview">
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                                            ...
                                            <div class="form-group-feedback form-group-feedback-right">
                                                <div class="form-check form-check-switchery">
                                                    <label class="form-check-label">
                                                        <input type="checkbox" id="status" class="form-check-input-switchery" @if($category->status === 1) checked="checked" @endif name="status" data-fouc>
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
