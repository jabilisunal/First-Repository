...
@section('content')
    @push('breadcrumb-buttons')
        <div class="page-title d-flex">
            <h4>
                <a class="text-black-50" href="{{route('ecommerce.facilities.index')}}"> Xüsusiyətlər</a>
            </h4>
            <a href="#" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
        </div>
        @can('content:post-create')
        <div class="header-elements d-none">
            <a href="{{route('ecommerce.facilities.create')}}" type="button" class="btn bg-teal-400 btn-labeled btn-labeled-left">
                <b><i class="icon-plus3"></i></b> Xüsusiyət əlavə et
            </a>
        </div>
        @endcan
    @endpush
                    ...
                    <table class="table datatable-button-html5-columns">
                        ...
                        @foreach($facilities as $key => $value)
                            <tr>
                                ...
                                <td>{{\App\Models\Facility::TYPES[$value->type]}}</td>
                                <td>{{$value->name}}</td>
                                <td>
                                    @if($value->status === 1)
                                ...
                                <td>
                                    <div class="list-icons">
                                        @can('content:post-show')
                                            <a target="_blank" href="{{route('ecommerce.facilities.show', [$value->id])}}" class="list-icons-item"><i class="icon-eye"></i></a>
                                        @endcan
                                        @can('content:post-edit')
                                            <a href="{{route('ecommerce.facilities.edit', [$value->id])}}" class="list-icons-item"><i class="icon-pencil7"></i></a>
                                        @endcan
                ...
                $(document).on('click', '.btn-remove', function () {
                    ...
                    swalInit({
                        ...
                        if(result.value) {
                            ...
                            $.ajax({
                                type: 'DELETE',
                                url: '/admin/ecommerce/facilities/'+id,
                                data: {
                                    _token: $('meta[name="csrf_token"]').attr('content')
                                },
                                success: function (data) {
                                    if (data.status) {
                                        window.location='{{route('ecommerce.facilities.index')}}'
                                    } else {
