...
@section('content')
    @push('breadcrumb-buttons')
        <div class="page-title d-flex">
            <h4>
                <a class="text-black-50" href="{{route('ecommerce.facilities.index')}}"> Xüsusiyətə düzəliş et</a>
            </h4>
    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{{route('ecommerce.facilities.update', [$facility->id])}}" method="post" enctype="multipart/form-data" id="submit-form">
                            ...
                            <div class="tab-pane fade show active" id="general">
                                <fieldset class="mb-3">
                                    ...
                                    <div class="form-group row">
                                        <label for="type" class="col-form-label col-lg-2 font-weight-semibold @if($errors->has('type')) text-danger @endif">
                                            Xüsusiyət Tipi
                                        </label>
                                        <div class="col-lg-10">
                                            <div class="form-group-feedback form-group-feedback-right">
                                                <select id="type" class="select-search @if($errors->has('type')) border-danger @endif" name="type">
                                                    <option value="" @if(old('type') === '') selected @endif>Seçilməyib</option>
                                                    @foreach($facilityTypes as $key => $facilityType)
                                                        <option value="{{$key}}" @if($facility->type === (int) $key) selected @endif>{{$facilityType}}</option>
                                                    @endforeach
                                                </select>
                                                @if($errors->has('type'))
                                    ...
                                    @if($facility->base_image->path)
                                        <div class="form-group row">
                                            <div class="col-form-label col-lg-2 font-weight-semibold"></div>
                                            <div class="col-lg-10">
                                                <div class="form-group-feedback form-group-feedback-right">
                                                    <img style="width: 200px; height: 100px; object-fit: contain;" src="/storage/{{$facility->base_image->path}}" alt="Image Preview">
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                                            ...
                                            <div class="form-group-feedback form-group-feedback-right">
                                                <div class="form-check form-check-switchery">
                                                    <label class="form-check-label">
                                                        <input type="checkbox" id="status" class="form-check-input-switchery" @if($facility->status === 1) checked="checked" @endif name="status" data-fouc>
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
                                                    ...
                                                    <div class="col-lg-10">
                                                        <div class="form-group-feedback form-group-feedback-right">
                                                            <input name="name[{{$language->code}}]" id="name_{{$language->code}}"  type="text"
                                                                   value="{{(old('name')[$language->code] ?? null) ?? ($translations[$language->code]['name'] ?? '')}}"
